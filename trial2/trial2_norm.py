...
class Tcsv:
	...
	def filtr(self, value: list, col_ind: Optional[int] = None):
		...
		- To filter "Toyota" and "Mazda" in column 2: Tcsv().filter(["Toyota", "Mazda],
		2).tolist()
		...
		# Convert self.reader to a set for faster filtering
		
		elif col_ind is None:
			self.reader = (row for row in self.reader if
						   any(cell.strip() in value for cell in row))
		
		# Filters value(s) if column index entered
		else:
			self.reader = [row for row in self.reader for values in value if
						   row[col_ind] == values]
	...
	def repv(self, values_dict: dict, col_ind: Optional[int] = None):
		...
		replaced = Tcsv(csv_data).repv({"Facey": "Facebook", "Redthis":
		"Reddit"}.tolist()
