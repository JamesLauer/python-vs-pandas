...
class TestTcsvGenViewr(unittest.TestCase):
	def setUp(self) -> None:
	...
	def test_topr_with_top_rows(self):
		...
		test_func = TcsvGen(self.csv_path).topr(4).tolist()
		self.assertEqual(test_data, test_func)


class TestTcsvGenFilter(unittest.TestCase):
	def setUp(self) -> None:
		self.csv_path = "unit_test_data/csv_data.csv"
	...
	def test_filtr_single_value_all_col(self):
		...
		test_data = get_mock_csv_path(filepath=mock_data_path)
		values = ['software']
		test_func = TcsvGen(self.csv_path).filtr(values).tolist()
		self.assertEqual(test_data, test_func)
	...
	def test_filtr_multi_value_single_col(self):
		...
		test_data = get_mock_csv_path(filepath=mock_data_path)
		values = ['seed', 'a']
		test_func = TcsvGen(self.csv_path).filtr(values, 9).tolist()
		self.assertEqual(test_data, test_func)
...
class TestTcsvGenDelCol(unittest.TestCase):
	def setUp(self) -> None:
		self.csv_path = "unit_test_data/csv_data.csv"
	...
	def test_delc_single_col(self):
		...
		test_data = get_mock_csv_path(filepath=mock_data_path)
		index = [2]
		test_func = TcsvGen(self.csv_path).delc(index).tolist()
		self.assertEqual(test_data, test_func)
...
class TestTcsvGenRepv(unittest.TestCase):
	def setUp(self) -> None:
		self.csv_path = "unit_test_data/csv_data.csv"
	def test_repv_single_value_all_col(self):
		...
		test_func = TcsvGen(self.csv_path).repv(values).tolist()
		self.assertEqual(test_data, test_func)
	...
	def test_repv_multi_values_single_col(self):
		...
		index = 0
		test_func = TcsvGen(self.csv_path).repv(values, index).tolist()
		self.assertEqual(test_data, test_func)
...
class TestTcsvGenChain(unittest.TestCase):
	def setUp(self) -> None:
		self.csv_path = "unit_test_data/csv_data.csv"
	...
	def test_chain_1(self):
		...
		dvals = [2, 5]
		test_func = TcsvGen(self.csv_path).filtr(fvals).repv(rvals).topr(3).delc(dvals).tolist()
		self.assertEqual(test_data, test_func)
	...
	def test_chain_2(self):
		...
		rvals = {'web': 'interweb'}
		dvals = [4, 6]
		test_func = TcsvGen(self.csv_path).topr(7).delc(dvals).repv(rvals).filtr(fvals).tolist()
		self.assertEqual(test_data, test_func)
	...
	def test_chain_3(self):
		...
		rvals = {'seed': 'pod'}
		dvals = [0]
		test_func = TcsvGen(self.csv_path).filtr(fvals).repv(rvals).delc(dvals).tolist()
		self.assertEqual(test_data, test_func)
	...
	def test_chain_4(self):
		...
		rindex = 1
		dvals = [0]
		test_func = TcsvGen(self.csv_path).filtr(fvals, findex).repv(rvals, rindex).delc(
			dvals).tolist()
		self.assertEqual(test_data, test_func)